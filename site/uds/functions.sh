...
fix_launcher() {
    sudo rm -f /usr/local/share/custom-launchers/$(basename $script_url|cut -d '.' -f1)
    cat <<EOF |sudo tee /usr/local/share/custom-launchers/$(basename $script_url|cut -d '.' -f1)>/dev/null
...
install_appimage() {
    ...
    basename=$(basename $script_url|cut -d '.' -f1)
    chmod +x $appimage
