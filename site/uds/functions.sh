...
fix_launcher() {
    sudo rm -f /usr/local/share/custom-launchers/$(basename ${BASH_SOURCE[1]}|cut -d '.' -f1)
    cat <<EOF |sudo tee /usr/local/share/custom-launchers/$(basename ${BASH_SOURCE[1]}|cut -d '.' -f1)>/dev/null
...
install_appimage() {
    ...
    basename=$(basename ${BASH_SOURCE[1]}|cut -d '.' -f1)
    chmod +x $appimage
